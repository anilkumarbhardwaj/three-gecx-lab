...
(`sharmajyoti-931027-org`), using Cloud Identity as IdP and a personal
...
2. Log in and confirm the org and billing account:
   ```bash
   gcloud auth login
...
```bash
cd 0-bootstrap
cp terraform.tfvars.example terraform.tfvars  # Change Org, project etc 
terraform init
terraform apply -target=google_project.cicd
gcloud services enable serviceusage.googleapis.com --project=<prefix>-gecx-cicd
terraform apply
```
